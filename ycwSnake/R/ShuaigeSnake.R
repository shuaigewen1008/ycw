...
stage1<-function(){
  ...
  fail<-function(){
    ...
    if(length(which(e$head<1))>0 | length(which(e$head>e$width))>0){  # ????????????????????
      ...
      keydown('q')
      play("F:/qimozuoye/ycwSnake/error.mp3")
      return(TRUE)
    }
  ...
  # snake body
  body<-function(){
    e$m[e$lastx,e$lasty]<-0 # 原来的头的值变成0
    ...
    if(length(index(e$col_furit))<=0){ # 如果蛇吃掉水果后
      ...
      play("F:/qimozuoye/ycwSnake/Score.mp3")
    }
...
stage0<-function(){
  ...
  text(0.2,0.05,label="Author:DanZhang(shuaigewen)",cex=1)
